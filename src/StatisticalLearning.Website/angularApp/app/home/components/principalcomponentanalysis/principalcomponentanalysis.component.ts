...
import { PrincipalComponentAnalysisResult, PrincipalComponent } from '../../../stores/statistic/models/pca.result';
...
export class PrincipalComponentAnalysisComponent implements OnInit {
    principalComponentAnalysisResult: PrincipalComponentAnalysisResult;
    names: string[] = [ "100m", "Long jump", "Short jump", "High jump", "400m", "100m hurdle", "Discus", "Pole vault", "Javeline", "1500m" ];
    playerNames: string[] = [
        "SEBRLE",
        "CLAY",
        "KARPOV",
    ...
    ngOnInit(): void {
        this.store.pipe(select(fromAppState.selectPrincipalComponentAnalysisResult)).subscribe((principalComponentResult: PrincipalComponentAnalysisResult) => {
            if (!principalComponentResult) {
                return;
            }


            this.principalComponentAnalysisResult = principalComponentResult;
            const pc1 = principalComponentResult.principalComponents[0];
            const pc2 = principalComponentResult.principalComponents[1];
            ...
            {
                const name = this.playerNames[j];
                indivData.push({
                    node: 'markers',
                    name: name,
                    x: [this.principalComponentAnalysisResult.transformed[j][0]],
                    y: [this.principalComponentAnalysisResult.transformed[j][1]]
                });
            }
    ...
    displayInfo() {
        const data = new PrincipalComponentDialogView();
        var i = 0;
        this.principalComponentAnalysisResult.principalComponents.forEach(function (p: PrincipalComponent) {
            const record = new PrincipalComponentView();
...
class PrincipalComponentView extends PrincipalComponent {
    constructor() {
        super();
    }
...
class PrincipalComponentDialogView {
    constructor() {
        this.components = [];
    }

    components: PrincipalComponentView[];
    data: number[][];
}
...
export class PrincipalComponentAnalysisDialog {
    displayedColumns: string[] = ["name", "cumulative", "eigenValue", "proportion"];
    matrixDisplayedColumns: string[] = ["100m", "longJump", "shortJump", "highJump", "400m", "100mHurdle", "discus", "poleVault", "javeline", "1500m"];

    constructor(public dialogRef: MatDialogRef<PrincipalComponentAnalysisDialog>, @Inject(MAT_DIALOG_DATA) public data: PrincipalComponentDialogView) {
