...
import { LinearDiscriminantAnalysisResult } from '../../../stores/statistic/models/lda.result';
var Plotly = require('plotly.js-dist');

@Component({
    selector: 'lineardiscriminantanalysis-component',
    templateUrl: './lineardiscriminantanalysis.component.html',
    styleUrls: ['./lineardiscriminantanalysis.component.scss']
})
export class LinearDiscriminantAnalysisComponent implements OnInit {
    linearDiscriminantAnalysisResult: LinearDiscriminantAnalysisResult;
    input: number[][] = [
        [ 5.1, 3.5, 1.4, 0.2 ],
        [ 4.9, 3.0, 1.4, 0.2 ],
        [ 4.7, 3.2, 1.3, 0.2 ],
    ...
    uniqueNames: string[] = ['Iris-setosa', 'Iris-versicolor', 'Iris-virginica']

    constructor(private store: Store<fromAppState.AppState>) { }

    ngOnInit(): void {
        this.store.pipe(select(fromAppState.selectLinearDiscriminantAnalysisResult)).subscribe((ldaResult: LinearDiscriminantAnalysisResult) => {
            if (!ldaResult) {
                return;
            }
            ...
            for (let i = 0; i < this.uniqueNames.length; i++) {
                const uniqueName = this.uniqueNames[i];
                const record: any = {
                    type: 'scatter',
                    mode: 'markers',
                    name: uniqueName,
                ...
                for (let j = 0; j < this.output.length; j++) {
                    const name = this.output[j];
                    const transformed = ldaResult.transformed[j];
                    if (name === uniqueName) {
                        record.x.push(transformed[0]);
                        record.y.push(transformed[1]);
                    }
                }

                indivData.push(record);
            }

            Plotly.newPlot('ldaIndivChartContainer', indivData);
        });
        this.refresh();
    }
    ...
    refresh() {
        this.store.dispatch(new fromStatisticActions.ComputeLinearDiscriminantAnalysis(this.input, this.output));
    }
}